...
def add_seconds(_deg, _dec_m):
    """
    Strips a coordinate containing degrees and decimal minutes to degrees, minutes and seconds.
    :param _deg: input coordinate degrees.
    :param _dec_m: input coordinate decimal minutes.
    :return: a coordinate with degrees, minutes and seconds.
    """
    _m = int(_dec_m)
    _s = (_dec_m % max(1, _m)) * 60.0
    return _deg, _m, _s
...
def footpoint_latitude(y):
    ...
    result = (
        y_ + (beta_ * sin(2.0 * y_)) + (gamma_ * sin(4.0 * y_))
        + (delta_ * sin(6.0 * y_))
        + (epsilon_ * sin(8.0 * y_))
    )
...
class DMSElement:
    ...
    def __init__(self, d, m, s, hemisphere):
        self.degrees = d
        self.minutes = m
        self.seconds = s
...
def wgs_to_dms(wgs):
    ...
    long_ms = (wgs.get_longitude_degrees() - lon_deg) * 60.0 * 60.0
    long_m = int(long_ms / 60)
    long_s = long_ms % 60

    # Build coordinate object
    return DMSCoordinate(lat_deg, lat_m, lat_s, lon_deg, long_m, long_s)
...
def utm_to_wgs(utm):
    ...
    * to optimize computations.
    ...
    x3f = 1.0 / (6.0 * nfpow * cf)
        
    nfpow *= nf     # now equals Nf^4
    x4f = tf / (24.0 * nfpow)
        
    nfpow *= nf     # now equals Nf^5
    x5f = 1.0 / (120.0 * nfpow * cf)
        
    nfpow *= nf     # now equals Nf^6
    x6f = tf / (720.0 * nfpow)
        
    nfpow *= nf     # now equals Nf^7
    x7f = 1.0 / (5040.0 * nfpow * cf)
        
    nfpow *= nf     # now equals Nf^8
    x8f = tf / (40320.0 * nfpow)
    ...
    # Calculate latitude
    lat = (phif + x2f * x2poly * (x * x)
           + x4f * x4poly * pow(x, 4.0)
           + x6f * x6poly * pow(x, 6.0)
           + x8f * x8poly * pow(x, 8.0)
    )
        
    # Calculate longitude
    lon = (lambda0 + x1f * x
           + x3f * x3poly * pow (x, 3.0)
           + x5f * x5poly * pow (x, 5.0)
           + x7f * x7poly * pow (x, 7.0)
    )
