...
from src.ski.gps import GPSPoint
...
class GPXFile:
    ...
    tz = None  # Should use OS TZ as default
...
def __parse_trkpt(gpx_file, tp):
    ...
    if tp.nodeType == Node.ELEMENT_NODE:
        ...
        p.ts = datetime.strptime(__read_text(tp, "time"), "%Y-%m-%dT%H:%M:%SZ")\
            .replace(tzinfo=timezone("UTC")).astimezone(gpx_file.tz)
...
def parse_gpx_file(file_name, gpx_file=GPXFile()):
    ...
    f = open(file_name)
    doc = parse(f)
    gps = list(map(lambda e: __parse_trkpt(gpx_file, e), doc.getElementsByTagName("trkpt")))
    f.close()
