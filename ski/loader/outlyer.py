...
def is_outlyer(prev_point, point):
    ...
    log.debug('is_outlyer: ts_delta=%d, calc_dist=%.2f, gps_speed=%.2f, calc_speed=%.4f, calc_spd_factor=%.3f',
    ...
    if calc_spd > outlyer_max_speed:
        # Record in pointlog
        log_point(point.ts, 'Removed as outlyer', ts=point.ts, calc_spd=calc_spd, calc_spd_factor=calc_spd_factor)
        return True
